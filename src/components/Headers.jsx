  ...
  return (
    <div className={isScrolled ? 'sticky' : 'header'}>
      <div className="container header__container">
        <Link to={ user ? '/news' : '/' } className={isScrolled ? 'sticky__logo' : 'header__logo'}>
        ...
        <nav className="header__nav">
          <ul className={isOpen ? 'openList' : 'header__list'}>
            ...
            {/* <li 
                      onClick={() => setIsOpen(false)}
                      className="header__item">
                        <Link to='/news/:id' className="header__link">
                            New
                        </Link>
                    </li> */}
            {/* <li 
            ...
            {user
              ? <div className="header__user">
                <img src={user.avatar} alt="user" className="header__user__image" />
                <div className="header__user__text">
                  <h4 className="header__user__name">{user.name}</h4>
                  <button onClick={logout} className="header__user__logout">
